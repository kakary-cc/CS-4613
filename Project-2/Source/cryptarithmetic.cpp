...
private:
    ...
    vector<pair<int, int> > constraints;  /* Equality constraints */
public:
    CSP() : degree{3, 4, 4, 3, 3, 4, 4, 3, 4, 3, 4, 4, 3, 7, 8, 7} {
        for (int i = 0; i < 16; i++) {
            assignment[i] = -1;  /* Dummy value for unassigned variables */
            if (i >= c1) {
                remaining_values[i].push_back(1);
                remaining_values[i].push_back(0);
            }
            else if (i == x1 || i == x5) {
                for (int j = 9; j >= 1; j--)
    ...
    void append_constraint(int i, int j) {
        constraints.push_back(make_pair(i, j));
        degree[i]++;
        degree[j]++;
    }
    ...
    int select_unassigned_variable() {
        ...
        for (int i = 0; i < 16; i++) {  /* Minimum remaining values */
            if (assignment[i] == -1 && remaining_values[i].size() < min) {
    ...
    friend ostream& operator<<(ostream& out, const CSP &csp) {
        out << csp.assignment[x1] << csp.assignment[x2] << csp.assignment[x3] << csp.assignment[x4] << endl;
        out << csp.assignment[x5] << csp.assignment[x6] << csp.assignment[x7] << csp.assignment[x8] << endl;
        out << csp.assignment[x9] << csp.assignment[x10] << csp.assignment[x11] << csp.assignment[x12] << csp.assignment[x13] << endl;
        return out;
    }
};

bool backtrack(CSP &csp) {
    if (csp.is_complete()) {
        cout << csp;
        return true;
    }
    ...
    for (int val = csp.order_domain_values(var); val != -1; val = csp.order_domain_values(var)) {
        ...
        if (new_csp.is_consistent()) {
            if (backtrack(new_csp))
                return true;
        }
        // if consistent
        // add to assignment
        // if backtrack, return
        // else remove from assignment
    }
    return false;
}
